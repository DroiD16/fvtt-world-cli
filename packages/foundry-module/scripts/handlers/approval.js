      ...
      if (report.status === "unknown") {
        throw createBridgeError(
          ...
          `The GM client holding this bridge has no approval state for approvalId ${approvalId}. Approval ` +
            "state is runtime state: it does not survive a reload of that client, and a retained outcome " +
            "expires. The outcome is therefore indeterminate — the command may never have started, may be " +
            "running now, or may have completed and changed the world. Read the documents the command would " +
            "have written before anything else, report what you found, and if you decide to send the command " +
            "again, send it under a fresh idempotency key.",
