...
permissions window does not show them, because there is nothing about them to change. Pairing and
the other `auth` operations are answered by the daemon and never reach the module, so no command
